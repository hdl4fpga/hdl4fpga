...
:ref:`hz_div` as a base to display the corresponding values on the display. The
steps to fill up hz_scales are the following:
...
D
    Get the corresponding ascii factor character and fill it in :ref:`deca`. In
    the example: the corresponding factor is u.
E
...
F
    ...
    horizontal base division value on the display. The :ref:`scale` member is a four
