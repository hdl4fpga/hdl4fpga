...
---------------------
 ...
 fpgaser.inf           INF file which may be used with Windows XP.
...
Changes from version 2011-10-04 to 2013-12-05
---------------------------------------------

* usb_transact.vhd:
  + Fix bug in handling of echo in RX channel from our TX packets.
    (Thanks to Nicolas Bouquet for reporting this issue.)


Changes from version 2009-10-06 to 2011-10-04
---------------------------------------------

* usb_serial.vhd:
  + Add support for manufacturer/product/serial strings in USB descriptor.
  + Fix assertion statement (issue reported by Werner Stern and Martin Neumann).
  + Reset s_bufptr (missing reset reported by Werner Stern and Martin Neumann).

* te0146.ucf:
  + Add zero hold time constraint on signals from USB3250
    (Theodor Kranz reported problems when I/O flip-flops are not used).

* perftest.c
  + Fix bug in "-s" option parsing (fixed by Joe Rothweiler).
  + Move call to libusb_handle_events() to prevent unnecessary waiting
    (as recommended by Joe Rothweiler).
...
Version 20131130, Intel DH67CF, Linux 3.12.0, ehci:

Performance FS, /dev/ttyACM0:
  TX   128000 + RX 16000000 in 15.020 s =  1073778 bytes/s
  RX 67108864 bytes in 61.659 s =  1088382 bytes/s
  TX 64000000 bytes in 65.602 s =   975583 bytes/s

Performance FS, libusb-1.0 async:
  RX 67108864 bytes in 59.234 s =  1132944 bytes/s
  TX 64000000 bytes in 62.597 s =  1022414 bytes/s

Performance HS, /dev/ttyACM0:
  TX   128000 + RX 16000000 in  0.502 s = 32130299 bytes/s
  RX 67108864 bytes in  1.491 s = 45002295 bytes/s
  TX 64000000 bytes in  1.463 s = 43755811 bytes/s
  lost 3103708 out of 67108864 bytes = 4.625 %

Performance HS, libusb-1.0 async:
  RX 67108864 bytes in  1.490 s = 45035364 bytes/s
  TX 64000000 bytes in  1.427 s = 44849240 bytes/s
  lost 56263788 out of 67108864 bytes = 83.840 %
...
* People say that Windows may crash when attaching two USB devices with
  identical serial numbers. (Not tested.)

* Linux cdc-acm driver can not properly throttle a fast USB device.
  Received data are accepted very quickly in USB completion callback,
  and only later will the n_tty ldisc discover that it's buffer is almost
  full and call acm_tty_throttle(). By that time the intermediate 64k tty
  buffer is already overflowing.
  Also, it looks like acm_read_bulk_callback() sets the read_urbs_free bit
  too early, but fixing this does not solve anything.

* Nicolas Bouquet reports that there are some UTMI chips which assert
  RX_ACTIVE during transmission AND ALSO keep RX_ACTIVE asserted during
  the very short time between end of transmission and beginning of incoming
  ACK handshake. In particular SMSC 3250.
  My ACK detection does not work with these chips and I see no way to fix it.
  See also http://www.usb.org/developers/presentations/pres0602/peter_tengstrand.pdf
